...
offset = 0.1 + 0.003499998807875214 # 0.1 + 0.003499998807875214 
...
desiredFramePlacement = M_ct #robot.pin_robot.data.oMf[id_endeff] 
...
# Contact model
ref_placement = crocoddyl.FramePlacement(id_endeff, M_ct) # robot.pin_robot.data.oMf[id_endeff]
...
for i in range(N_h):
  ...
  # Add cost models
  runningModels[i].differential.costs.addCost("placement", framePlacementCost, 7e-1) 
#   runningModels[i].differential.costs.addCost("velocity", frameVelocityCost,  1e-4, active=True) 
  runningModels[i].differential.costs.addCost("force", frameForceCost, 5, active=False) 
...
#   runningModels[i].differential.costs.addCost("ctrlLim", uLimitCost, 1) 
  runningModels[i].set_w_reg_lim_costs(1e-4, u_reg_ref, 0, u_lim_ref)
  # Add armature
...
# Add cost models
terminalModel.differential.costs.addCost("placement", framePlacementCost, 1e5) 
# terminalModel.differential.costs.addCost("frictionCone", frictionConeCost, 1e-1, active=False)
terminalModel.differential.costs.addCost("stateReg", xRegCost, 1e-4) 
...
for i in range(N_tot): 
    ...
    if(i%int(ctrl_freq/plan_freq) == 0):
        ...
        # STATE-based switch
        if(len(p.getContactPoints(1, 2))>0): 
            print("      !!! CONTACT !!! ")
            # Terminal model
            ...
            ddp.problem.terminalModel.differential.costs.costs["placement"].weight = 1e-2
            # ddp.problem.terminalModel.differential.costs.changeCostStatus("force", True)
            # ddp.problem.terminalModel.differential.costs.changeCostStatus("velocity", True)
            # ddp.problem.terminalModel.differential.costs.changeCostStatus("placement", False)
            # if(contact==False):
            ...
            for k,m in enumerate(ddp.problem.runningModels[:]):
                ...
                m.differential.costs.costs["stateReg"].reference = X_mea[i,:nq+nv]
                # m.differential.costs.costs["ctrl"].reference = X_mea[i,:nq+nv]
                # m.differential.costs.changeCostStatus("frictionCone", True)
                # m.differential.costs.changeCostStatus("velocity", True)
                # m.differential.costs.changeCostStatus("placement", False)
                if(contact==False):
            ...
            # Terminal model
            ddp.problem.terminalModel.differential.contacts.contacts["contact"].contact.Mref.placement = robot.pin_robot.data.oMf[id_endeff]
            ddp.problem.terminalModel.differential.contacts.changeContactStatus("contact", False)
            # ddp.problem.terminalModel.differential.costs.changeCostStatus("force", False)
            # ddp.problem.terminalModel.differential.costs.changeCostStatus("placement", True)
            # Running models
            for k,m in enumerate(ddp.problem.runningModels[:]):
                m.differential.contacts.contacts["contact"].contact.Mref.placement = robot.pin_robot.data.oMf[id_endeff]
                m.differential.contacts.changeContactStatus("contact", False)
                m.differential.costs.changeCostStatus("force", False)
                # m.differential.costs.changeCostStatus("frictionCone", False)
                # m.differential.costs.changeCostStatus("placement", contact_is_active)
