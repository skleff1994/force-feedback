...
The goal of this script is to exhibit issues when no low-level torque tracking 
is available. The actuation is modeled as scaling+noise+delay on desired torques
Simulation frequency = control frequency = 1kHz
'''
...
simu_freq = config['ctrl_freq']  # simulation at the control frequency here
dt_simu = 1./simu_freq
...
delay_cycle = int(delay_ms * 1e-3 * simu_freq)
buffer = []
  # Moving avg filter
...
for i in range(N_simu): 
    print("  ")
    print("SIMU/CTRL step "+str(i)+"/"+str(N_simu))

    # Solve OCP if we are in a planning cycle (MPC frequency)
    ...
    # Simulate actuation step simulator (control & low-level control frequency)
    X_des[i+1, :] = x_des             # Record predicted state
    U_des[i, :] = u_des               # Record desired control
