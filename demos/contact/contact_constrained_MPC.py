...
@file contact_constrained_MPC.py
...
from classical_mpc.ocp import OptimalControlProblemClassicalWithConstraints, OptimalControlProblemClassical
...
def solveOCP(q, v, ddp, nb_iter, node_id_reach, target_reach, node_id_contact, force_weight, TASK_PHASE, target_force):
        t = time.time()
...
#   ddp = OptimalControlProblemClassical(robot, config).initialize(x0, callbacks=False)
  ...
  for k,m in enumerate(models):
      ...
      m.differential.costs.costs["velocity"].active = False
      m.differential.costs.costs["stateLim"].active = False
      if(k < config['N_h']):
...
#   bench = MPCBenchmark()
  ...
  for i in range(sim_data.N_simu): 
    ...
    if(i%int(sim_data.simu_freq/sim_data.plan_freq) == 0):    
        ...
        solveOCP(q, v, ddp, config['maxiter'], node_id_reach, target_position, node_id_contact, force_weight, TASK_PHASE, target_force)
