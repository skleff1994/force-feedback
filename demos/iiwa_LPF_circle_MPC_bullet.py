...
for i in range(sim_data['N_simu']): 
    ...
    # # If sim time coincides with OCP shooting node time shift tracking problem cost references
    # if(i%int(1./OCP_TO_SIMU_RATIO)==0):
    #     logger.debug('reset tracking pb nb_plan='+str(nb_plan)+' , i='+str(i))
    ...
    #     # ocp_utils.set_ee_tracking_problem(ddp, EE_ref, mpc_cycle=nb_plan, ratio=OCP_TO_PLAN_RATIO)   

  #  If nb plan time coincides with OCP shooting node time shift tracking problem cost references
    if(nb_plan%int(1./OCP_TO_PLAN_RATIO)==0):
        logger.debug('reset tracking pb nb_plan='+str(nb_plan)+' , i='+str(i))
        shift = int(nb_plan*OCP_TO_PLAN_RATIO)
        logger.debug("SHIFT = "+str(shift))
        for k,m in enumerate(list(ddp.problem.runningModels) + [ddp.problem.terminalModel]):
          if(k+shift < EE_ref.shape[0]):
            ref = EE_ref[k+shift]
  ...
  # Solve OCP if we are in a planning cycle (MPC/planning frequency)
    if(i%int(freq_SIMU/freq_PLAN) == 0):
        for k,m in enumerate(list(ddp.problem.runningModels) + [ddp.problem.terminalModel]):
          ...
          if(k+1 < EE_ref.shape[0]):
            ref_curr = EE_ref[k]
            ref_next = EE_ref[k+1]
            ref      = ref_curr + OCP_TO_PLAN_RATIO * (ref_next - ref_curr)
