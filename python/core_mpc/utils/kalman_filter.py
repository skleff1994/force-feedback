...
class ExtendedKalmanFilter:
    '''
    Extended Kalman filter environment class
    '''
    ...
    def __init__(self, Q, R):
        ...
        # State transition model : linearization
        # x(n+1) = Ax(n) + Bu(n) + w(n) where w(n)~N(0,Q)
        # self.A = model.Ad
        # self.B = model.Bd
        self.Q = Q
        ...
        self.R = R
        # Dimensions
        self.nx = 14 #self.A.shape[0]
        self.nu = 7  #self.B.shape[1]
        self.ny = 7+3 #self.H.shape[0]
    
    def step(self, x, P, u, y, A, B, H):
        '''
        Take a filtering step : predict + update
         Input:
          - x : previous state estimate      : x^+(k-1)
        ...
        '''
        # Predict (state and covariance based on previous estimates and model)
        x_pred, P_pred = self.predict(x, P, u, A, B, H)
        # Update predictions based on latest measurement
        x_next, P_next, K, y_err = self.update(x_pred, P_pred, y, A, B, H)
        return x_next, P_next, K, y_err

    def predict(self, x_post_prev, P_post_prev, u_prev, A, B, H):
        '''
        Prediction step 
         Input:
          - x_post_prev : previous a posteriori state estimate      : x^+(k-1)
        ...
        '''
        # predict state estimate
        x_prio_curr = A.dot(x_post_prev) + B.dot(u_prev)
        # predict covariance estimate
        P_prio_curr = A.dot(P_post_prev).dot(A.T) + self.Q
        # Return predicted state estimate and covariance
        return x_prio_curr, P_prio_curr

    def update(self, x_prio_curr, P_prio_curr, y_currA, B, H):
        '''
        Update step 
         Input:
          - x_prio_curr : current a priori state estimate           : x^-(k)
        ...
        # update innovation
        y_res_curr = y_curr - H.dot(x_prio_curr)
        # update Kalman gain
        K = P_prio_curr.dot(H.T).dot(np.linalg.inv(self.R + H.dot(P_prio_curr).dot(H.T)))
        # update state estimate
        x_post_curr = x_prio_curr + K.dot(y_res_curr)
        # update covariance estimate
        P_post_curr = (np.eye(self.nx) - K.dot(H)).dot(P_prio_curr)
        # Return measurement residual, Kalman gain, updated state estimate and covariance
        return x_post_curr, P_post_curr, K, y_res_curr