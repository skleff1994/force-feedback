...
# # Define linear inverted pendulum model
# class LIPM:
...
#         # Inverted Pendulum constant
#         self.omega = np.sqrt(self.g / self.z_com)
        
#         # State space model (discretized)
...
#             u[:,i] = controller(x[:,i],i) #np.clip(controller(x[:,i],i), foot_position[i]-self.foot_size, foot_position[i]+self.foot_size)
