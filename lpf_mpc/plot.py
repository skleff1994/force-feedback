...
class DDPDataPlotterLPF(DDPDataPlotterAbstract):
    def __init__(self):
        return super().__init__()
    

    def plot_ddp_results_LPF(self, DDP_DATA, which_plots='all', labels=None, markers=None, colors=None, sampling_plot=1, SHOW=False):
        '''
        ...
        for k,data in enumerate(DDP_DATA):
            ...
            if(k==0):
                ...
                if('ee' in which_plots or which_plots =='all' or 'all' in which_plots):
                    if('xs' in data.keys()):
                        fig_ee_lin, ax_ee_lin = self.plot_ddp_endeff_LPF_linear(data, label=labels[k], marker=markers[k], color=colors[k], MAKE_LEGEND=make_legend, SHOW=False)
                        fig_ee_ang, ax_ee_ang = self.plot_ddp_endeff_LPF_angular(data, label=labels[k], marker=markers[k], color=colors[k], MAKE_LEGEND=make_legend, SHOW=False)
