...
class DAMSoftContactDynamics1(crocoddyl.DifferentialActionModelAbstract):
    '''
    ...
    def __init__(self, stateMultibody, actuationModel, costModelSum, frameId, Kp=1e3, Kv=60, oPc=np.zeros(3), pinRefFrame=pin.LOCAL):
        ...
        # hard coded costs : fz +10 N in pinRef frame
        self.f_des = np.array([0.,0.,10])
        self.f_weight = 0.
    ...
    def createData(self):
        ...
        '''
        data = DADSoftContactDynamics1(self)
        return data
    ...
    def calc(self, data, x, u):
        ...
        '''
        # logger.debug("Entering DAMSoft1.calc")
        q = x[:self.state.nq]
        v = x[self.state.nq:]
    ...
    def calcDiff(self, data, x, u):
        ...
        Compute derivatives 
        '''
        # logger.debug("Entering DAMSoft1.calcDiff")
        # First call calc
        q = x[:self.state.nq]
        v = x[self.state.nq:]
        oRf = data.pinocchio.oMf[self.frameId].rotation
        ...
        # logger.debug("YOYOYO")
        # # add hard-coded cost
        # data.costs.Lx += self.f_weight * self.f_residual.T @ data.df_dx 
        # data.costs.Lxx += self.f_weight * data.df_dx.T @ data.df_dx 


class DADSoftContactDynamics1(crocoddyl.DifferentialActionDataAbstract):
    '''
...
class DAMSoftContactDynamics2(crocoddyl.DifferentialActionModelFreeFwdDynamics):
    '''
    Computes the forward dynamics under visco-elastic (spring damper) force
    '''
    def __init__(self, stateMultibody, actuationModel, costModelSum, frameId, Kp=1e3, Kv=60, oPc=np.zeros(3), pinRefFrame=pin.LOCAL):
        ...
        self.armature = np.zeros(self.state.nq)
        self.oPc = oPc
        # To cannot overwrite DAMFree attributes I guess... 
        # self.actuation = actuationModel
        # self.costs = costModelSum
        # self.pinocchio = self.state.pinocchio
        # hard coded costs : fz +10 N in pinRef frame
    ...
    def createData(self):
        '''
            The data is created with a custom data class that contains the filtered torque tau_plus and the activation
        '''
        data = DADSoftContactDynamics2(self)
        return data
    ...
    def calc(self, data, x, u):
        ...
        Compute joint acceleration and costs 
         using visco-elastic force
        '''
        # logger.debug("Entering DAMSoft2.calc")
        q = x[:self.state.nq]
        v = x[self.state.nq:]
    ...
    def calcDiff(self, data, x, u):
        ...
        Compute derivatives 
        '''
        # logger.debug("Entering DAMSoft2.calcDiff")
        # First call calc
        q = x[:self.state.nq]
        v = x[self.state.nq:]
        oRf = data.pinocchio.oMf[self.frameId].rotation
...
class DADSoftContactDynamics2(crocoddyl.DifferentialActionDataFreeFwdDynamics):
    '''
    Creates a data class with differential and augmented matrices from IAM (initialized with stateVector)
    '''
    def __init__(self, am):
