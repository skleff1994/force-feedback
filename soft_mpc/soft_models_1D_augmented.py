...
class IAMSoftContactDynamics1D(crocoddyl.ActionModelAbstract): #IntegratedActionModelAbstract
    def __init__(self, dam, dt=1e-3, withCostResidual=True):
        ...
        crocoddyl.ActionModelAbstract.__init__(self, crocoddyl.StateVector(dam.state.nq + dam.state.nv + 1), dam.nu, dam.costs.nr + 1)
...
class DAMSoftContactDynamics1D(crocoddyl.DifferentialActionModelAbstract):
    ...
    '''
    def __init__(self, stateMultibody, actuationModel, costModelSum, frameId, Kp=1e3, Kv=60, oPc=np.zeros(3), pinRefFrame=pin.LOCAL, type=2):
    ...
    def calc(self, data, x, f, u):
        ...
        if(self.active_contact):
            ...
            fLOCAL = np.zeros(3) ; fLOCAL[self.mask] = f
