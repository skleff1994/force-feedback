...
from crocoddyl.libcrocoddyl_pywrap import __version__
from crocoddyl.deprecation import *
import crocoddyl
import pinocchio
import numpy as np
import time
...
def rotationMatrixFromTwoVectors(a, b):
    ...
    if crocoddyl.libcrocoddyl_pywrap.getNumpyType() == np.matrix:
...
class DisplayAbstract:
    ...
    def displayFromSolver(self, solver, factor=1.):
        ...
        if crocoddyl.libcrocoddyl_pywrap.getNumpyType() == np.matrix:
            ...
            crocoddyl.libcrocoddyl_pywrap.switchToNumpyMatrix()
            warnings.warn("Numpy matrix supports will be removed in future release", DeprecationWarning, stacklevel=2)
    ...
    def getForceTrajectoryFromSolver(self, solver):
        ...
        for i, data in enumerate(datas):
            ...
            if hasattr(data, "differential"):
                if isinstance(data.differential, crocoddyl.libcrocoddyl_pywrap.DifferentialActionDataContactFwdDynamics):
                    fc = []
...
class CallbackDisplay(crocoddyl.libcrocoddyl_pywrap.CallbackAbstract):
    def __init__(self, display):
        crocoddyl.libcrocoddyl_pywrap.CallbackAbstract.__init__(self)
...
class CallbackLogger(crocoddyl.libcrocoddyl_pywrap.CallbackAbstract):
    def __init__(self):
        crocoddyl.libcrocoddyl_pywrap.CallbackAbstract.__init__(self)
        self.xs = []
        self.us = []
